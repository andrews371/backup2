...
def odds_draw(url):  
    ...
    #flag para saber se carregou a página
    flag = False
    inicio = time.time()
    fim = time.time()
    ...
    while flag == False:
        ...
        try:
            WebDriverWait(driver, 45).until(EC.presence_of_element_located((By.CSS_SELECTOR, 'tr.lo:nth-child(9) > td:nth-child(1) > div:nth-child(1) > a:nth-child(2)')))
            time.sleep(3) 
            fim = time.time()
            if (fim - inicio) >= 30:
                # reiniciando a contagem de tempo
                inicio = time.time()
                continue
            else:
                flag = True
