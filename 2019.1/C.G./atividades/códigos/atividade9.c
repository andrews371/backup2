...
void desenha_cubo(int rot, int trans)
{
	...
	glTranslated(0, 0, -trans); // define a posição do objeto na cena
...
// interação com o usuário pelo teclado
void teclado(unsigned char key, int rot, int trans){
  rot = 0;
  trans = 0;
  if (key == 114 || key == 82){
    rot += 5;
    desenha_cubo(rot,trans);
  }
