...
/*
	Compilando este código-fonte pelo Terminal : gcc prog1.c -o prog1.exe -lglut -lGL -lGLU -lm
	Executando : ./prog1.exe
	Referência : https://processolinux.wordpress.com/2009/07/31/compilando-codigo-em-c-e-opengl-no-linux/
			OBS : Tem um arquivo de texto, chamado Makefile, ao qual serve para executar este programa. Significa que
			precisa colocar o comando acima, para executar este programa.
			Para executar o programa, usando o Makefile, basta colocar o seguinte comando : make run
			OBS 2 : Certifique-se que, tanto o Makefile quanto o código-fonte, estejam no mesmo diretório.
*/
