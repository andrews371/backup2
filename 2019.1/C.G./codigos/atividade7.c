...
void init(void){
  	...
  	glOrtho (-45, -25, -45, -15, -1, 1);
}

void eqReta(){
  int coordenadas[] = {-40,-40,-30,-20};
  float m = (coordenadas[3] - coordenadas[1])/(coordenadas[2] - coordenadas[0]);
  int aux = 0, yaux = coordenadas[1];
  ...
  glBegin(GL_POINTS);
    ...
    for(aux = coordenadas[0] + 1; aux <= coordenadas[2]; aux++){
      yaux = m*aux + (m*(-aux) + yaux); 
      glVertex2i(aux,yaux); 
...
void retaOpengl(){
  ...
  glBegin(GL_LINES);
  	glVertex2f(coordenadas[0],coordenadas[1]);  glVertex2f(coordenadas[2],coordenadas[3]);
  glEnd();
...
int main(int argc, char** argv){
  ...
  glutCreateWindow("Retas usando o OpenGL");
  init();
